...
function ReceiveStockTable(props: ReceiveStockTableProps) {
  ...
  const columns: TableProps<any>['columns'] = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <div>
          <Button type="text" className='text-blue-800' onClick={() => setEditFormVisible(true)}>編集</Button>
          <Button type="text" className='text-red-400' onClick={() => handleDelete(row.id)}>削除</Button>
        </div>
      ),
    },
  ];
  ...
  return <>
    <Table
      ...
      pagination={false}
    />
    <EditForm visible={editFromVisible} onClose={() => setEditFormVisible(false)} onRefresh={onRefresh} />
  </>;
}
